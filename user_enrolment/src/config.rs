...
//file used to get all the .env var once and reuse them later in the project
...
pub fn port() -> u16{
    ...
    var("PORT").expect("PORT is not set").parse::<u16>().ok().expect("PORT should be int")
}
...
pub fn smtp_port() -> u16{
    ...
    var("SMTP_PORT").expect("SMTP_PORT is not set").parse::<u16>().ok().expect("SMTP_PORT should be an interger")
}
